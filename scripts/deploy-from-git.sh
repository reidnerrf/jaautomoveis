...
# 🚀 DEPLOY DOCKER COM SSH - JA Automóveis (Clone do Git)
# ======================================================
...
# Configurações
GIT_REPO="https://github.com/reidnerrf/jaautomoveis.git"
PROJECT_DIR="jaautomoveis"
...
# Banner
echo -e "${PURPLE}"
echo "🚀 DEPLOY DOCKER COM SSH - JA Automóveis (Git Clone)"
echo "==================================================="
echo -e "${NC}"
...
clone_repository() {
    ...
    if [ -d "$PROJECT_DIR" ]; then
        ...
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            info "Removendo diretório existente..."
            rm -rf "$PROJECT_DIR"
        else
    ...
    # Entrar no diretório
    cd "$PROJECT_DIR"
    success "Entrando no diretório do projeto"
}
...
setup_ssh() {
    ...
    if ! command -v ssh &> /dev/null; then
        ...
        if command -v apt-get &> /dev/null; then
            ...
            error "Não foi possível instalar SSH automaticamente"
            exit 1
        fi
    fi
    
    # Verificar se SSH está rodando (Linux)
...
show_status() {
    ...
    echo ""
    echo -e "${BLUE}📁 Projeto clonado de:${NC}"
    echo -e "  • Repositório: $GIT_REPO"
...
# Função principal
main() {
    echo -e "${BLUE}🎯 Iniciando deploy Docker com SSH a partir do Git...${NC}"
    echo ""
