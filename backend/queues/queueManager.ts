...
class QueueManager extends EventEmitter {
  ...
  constructor(redisUrl?: string) {
    ...
    const hasRedisConfigured =
      clusterNodes.length > 0 || !!process.env.REDIS_URL || !!process.env.REDIS_HOST;
    ...
    if (clusterNodes.length > 0) {
      const nodes = clusterNodes.map((n) => {
        ...
        return { host, port: parseInt(portStr || "6379") } as any;
      });
  ...
  private createQueue(config: QueueConfig): Queue.Queue {
    ...
    const queue = new Queue(
      ...
      clusterNodes.length > 0
        ? {
            // Use cluster via createClient hook
            createClient: (_type: any) => {
              const nodes = clusterNodes.map((n) => {
                const [host, portStr] = n.split(":");
                return { host, port: parseInt(portStr || "6379") } as any;
              });
              return new Cluster(nodes, { redisOptions: { password: process.env.REDIS_PASSWORD } });
            },
            defaultJobOptions: {
