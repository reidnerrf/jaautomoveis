...
class DatabaseSharding {
  ...
  public getShard(strategy: 'hash' | 'roundRobin' | 'weight', key?: string): mongoose.Connection | null {
    switch (strategy) {
      case 'hash':
  ...
  public async getReplicaConnection(replicaName: string = 'main'): Promise<mongoose.Connection | null> {
  ...
  public async createVehicle(vehicleData: any, shardStrategy: 'hash' | 'roundRobin' | 'weight' = 'hash'): Promise<any> {
  ...
  public async findVehicle(vehicleId: string, shardStrategy: 'hash' | 'roundRobin' | 'weight' = 'hash'): Promise<any> {
  ...
  private calculateOptimalDistribution(currentDistribution: Map<string, number>): Map<string, number> {
    const totalDocuments = Array.from(currentDistribution.values()).reduce((sum, count) => sum + count, 0);
