...
const VehicleCarousel: React.FC<VehicleCarouselProps> = React.memo(
  ({ vehicles }) => {
    ...
    const prevSlide = useCallback(() => {
      setCurrentIndex((prev) =>
        prev === 0 ? Math.max(0, vehicles.length - visibleSlides) : prev - 1,
      );
    ...
    const carouselItems = useMemo(
      () =>
        vehicles.map((vehicle) => (
          <div
            key={vehicle.id}
            className="p-2 md:p-3 box-border"
    ...
    const indicators = useMemo(
      () =>
        Array.from(
          { length: Math.max(1, vehicles.length - visibleSlides + 1) },
          (_, i) => (
            <motion.button
              key={i}
    ...
    useEffect(() => {
      setCurrentIndex((prev) =>
        Math.min(prev, Math.max(0, vehicles.length - visibleSlides)),
      );
    ...
    return (
      <div className="relative w-full px-2 md:px-4" ref={containerRef}>
        ...
        {/* Indicadores */}
        <div className="flex justify-center mt-4 space-x-2">{indicators}</div>
      </div>
    );
  },
);
