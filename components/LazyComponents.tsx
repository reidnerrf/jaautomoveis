...
const VehicleCarouselSkeleton = () => (
  <div className="animate-pulse">
    ...
    <div className="flex space-x-2">
      {[...Array(4)].map((_, i) => (
        <div key={`carousel-skeleton-${i + 1}`} className="w-24 h-16 bg-gray-200 rounded"></div>
      ))}
    </div>
  </div>
);
...
const PriceComparisonSkeleton = () => (
  <div className="animate-pulse bg-gray-50 p-6 rounded-2xl">
    ...
    <div className="space-y-3">
      {[...Array(3)].map((_, i) => (
        <div key={`price-skeleton-${i + 1}`} className="h-4 bg-gray-200 rounded"></div>
      ))}
    </div>
  </div>
);
...
const ReviewsSkeleton = () => (
  <div className="animate-pulse">
    ...
    <div className="flex space-x-4">
      {[...Array(3)].map((_, i) => (
        <div key={`review-skeleton-${i + 1}`} className="flex-1 bg-gray-50 p-4 rounded">
...
const PageSkeleton = () => (
  <div className="animate-pulse">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {[...Array(6)].map((_, i) => (
        <div key={`page-skeleton-${i + 1}`} className="bg-white rounded-lg shadow p-4">
